...
#
# Run this whenever app.js, worker.js or rgbe.js changes, then commit and push.
set -euo pipefail
cd "$(dirname "$0")/.."
...
sed -i -E "s/(worker\.js\?v=)[0-9]+/\1$next/" selftest.html

echo "asset version $cur -> $next"
grep -nE '\?v=[0-9]+' index.html selftest.html
